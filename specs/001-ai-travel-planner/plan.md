...
기술 접근:
- 프론트엔드: Next.js 15 + React 19 (Progressive Web App 지원으로 모바일 준비)
- 백엔드: FastAPI + Python 3.11 (RESTful API로 모바일 앱과 공유 가능)
- AI 엔진: LangChain + OpenAI GPT-4
...
**Language/Version**:
- Frontend: TypeScript 5.x, Node.js 20.11+
- Backend: Python 3.11+
- Mobile Future: React Native 준비 (코드 재사용을 위한 공유 로직 구조)

**Primary Dependencies**:
- Frontend: Next.js 15, React 19, Tailwind CSS, Shadcn UI, React Query, Mapbox GL JS
- Backend: FastAPI, LangChain, OpenAI SDK, Pydantic, SQLAlchemy
...
**Constraints**:
- 예산 범위 내 일정 생성 (±10% 허용)
- 외부 API 의존성 관리 (장애 시 graceful degradation)
- 모바일 데이터 사용 최소화 (향후 앱 대비)
- GDPR 준수 (사용자 데이터 암호화, 삭제 권리)
- 한국어/영어 다국어 지원
...
**Phase 2 (Tasks)**: Pending - To be generated by `/speckit.tasks` command

---

**Next Steps**:
1. Generate OpenAPI specification in `contracts/api-spec.yaml`
